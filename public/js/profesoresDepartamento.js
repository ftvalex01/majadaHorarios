...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const modal2 = new bootstrap.Modal(
        document.getElementById("modulosProfesorModal")
    );
    const modulosProfesorContent = document.getElementById(
        "modulosProfesorContent"
    );

    async function loadProfesoresPorDepartamento(idDepartamento) {
        try {
            const response = await fetch(
                `https://majadahorarios-app.onrender.com/api/v1/departamentos/${idDepartamento}/profesores`,
                {
                    method: "GET",
                    headers: {
            ...
            if (!response.ok) {
                throw new Error(
                    "No se pudieron obtener los profesores del departamento"
                );
            }
            ...
            const profesoresListElement =
                document.getElementById("profesoresList");
            ...
            profesores.forEach((profesor) => {
                ...
                tarjetaProfesor
                    ...
                    .addEventListener("click", function () {
                        ...
                        document.getElementById(
                            "nombreProfesor"
                        ).textContent = `Nombre: ${nombreProfesor}`;
                        document.getElementById(
                            "email"
                        ...
                        let modal1 = new bootstrap.Modal(
                ...
                tarjetaProfesor
                    .querySelector(".ver-horario")
                    .addEventListener("click", async function () {
                        let profesorId = profesor.id;

                        try {
                            const response = await fetch(
                                `https://majadahorarios-app.onrender.com/api/v1/departamentos/${idDepartamento}/profesores/${profesorId}/modulos`,
                                {
                                    method: "GET",
                                    headers: {
                                        "Content-Type": "application/json",
                            ...
                            if (!response.ok) {
                                throw new Error(
                                    "No se pudo obtener el horario del profesor"
                                );
                            }
                            ...
                            console.log(
                                "modulo modal info" +
                                    JSON.stringify(horarioProfesor, null, 2)
                            );

                            // Mostrar información de todos los módulos en un nuevo modal
                            if (
                                Array.isArray(horarioProfesor) &&
                                horarioProfesor.length > 0
                            ) {
                                let infoModulos = "";
                                for (
                                    let i = 0;
                                    i < horarioProfesor.length;
                                    i++
                                ) {
                                    // sumas = [];
            ...
            `;
                                    ...
                                    <div class="accordion-item">
                                        ...
                                        <div id="collapse-${i}" class="accordion-collapse collapse" aria-labelledby="heading-${i}" data-bs-parent="#modulo-${i}-accordion">
                                            ...
                                            <p><strong>Distribución Horaria:</strong> ${distribucionHoraria.slice(
                                                1,
                                                -1
                                            )}</p>
                                ...
                                sumatorioHoras =
                                    sumarDistribucionesHorarias(sumas);
                                console.log(sumatorioHoras + "sumatorioHoras");
                                let sumatorioElement =
                                    document.createElement("p");
                                sumatorioElement.textContent = `Sumatorio de Horas: ${sumatorioHoras}`;
                                sumas = [];
                                console.log(
                                    horarioProfesor.reduce(
                                        (total, num) =>
                                            total +
                                            parseInt(num.distribucion_horas),
                                        0
                                    ) +
                                        "horarioProfesor.reduce((total, num) => total + parseInt(num.distribucion_horas), 0)"
                                );
                                if (
                                    sumatorioHoras >= 17 &&
                                    sumatorioHoras <= 20
                                    ...
                                    errorP.innerText =
                                        "Debe cumplir un mínimo de 17 horas y un máximo de 20";
                                    sumatorioElement.appendChild(errorP);
                                }
                                modulosProfesorContent.appendChild(
                                    sumatorioElement
                                );
                                sumatorioBool = true;
                                // }
                            ...
                            console.error(
                                "Error al obtener el horario del profesor:",
                                error
                            );
                            // Manejo de errores
                        }
            ...
            console.error(
                "Error al obtener profesores del departamento:",
                error
            );
            // Manejo de errores
        }
